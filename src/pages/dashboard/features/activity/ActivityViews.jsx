...
const ACTION_STYLES = {
  ...
  unassign: "bg-amber-50 text-amber-700",
  borrow: "bg-sky-50 text-sky-700",
...
function ActionBadge({ action }) {
  return (
    <span
      className={`inline-flex items-center whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-semibold ${ACTION_STYLES[action] || "bg-slate-100 text-slate-700"
        }`}
    >
          ...
          changes.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {changes.map((change) => (
                <span
                  key={change.key}
                  className="rounded-lg bg-white px-2.5 py-1.5 text-xs text-slate-600 ring-1 ring-slate-200"
                >
                  <span className="font-semibold text-slate-800">{humanizeFieldKey(change.key)}:</span>{" "}
                  <span className="text-rose-600 line-through">{formatFieldValue(change.from)}</span>{" "}
                  <span className="text-slate-400">&rarr;</span>{" "}
                  <span className="text-emerald-700">{formatFieldValue(change.to)}</span>
                </span>
              ))}
            </div>
          ) : (
  ...
  return (
    <>
      <tr className="transition hover:bg-slate-50/70">
        <td className="whitespace-nowrap px-4 py-3 text-slate-500">{formatTimestamp(entry.timestamp)}</td>
        {showActor && (
          <td className="whitespace-nowrap px-4 py-3 font-semibold text-slate-950">{entry.actorName}</td>
        )}
    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8">
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <div className="border-b border-slate-100 px-5 py-4">
          <h2 className="text-[15px] font-semibold text-slate-950">My activity</h2>
    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8">
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
          <div>
            <h2 className="text-[15px] font-semibold text-slate-950">Activity log</h2>
            <p className="mt-0.5 text-[13px] text-slate-500">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <input
              ...
              className="h-9 w-52 rounded-lg border border-slate-200 bg-white px-3 text-[13px] text-slate-700 outline-none transition hover:border-slate-300 focus-visible:ring-2 focus-visible:ring-orange-400 focus-visible:ring-offset-2 focus-visible:ring-offset-white"
            />
          </div>
        </div>
