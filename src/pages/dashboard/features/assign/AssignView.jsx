  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5">
      <div className="mb-4 flex items-start gap-3">
        <div className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-slate-950 text-xs font-bold text-white">
          {step}
        </div>
        <div>
  ...
  if (isFormDataLoading) {
    return (
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        <div className="rounded-xl border border-slate-200 bg-white px-5 py-10 text-center text-[13px] text-slate-500">
          Loading assign form...
        </div>
      </div>
    );
    ...
    return (
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center gap-3 rounded-xl border border-slate-200 bg-white px-5 py-10 text-center">
          ...
          <p className="text-[13px] font-semibold text-slate-700">Couldn&apos;t load the assign form</p>
          <p className="text-xs text-slate-500">{formDataError}</p>
          <button
            type="button"
  ...
  return (
    <div className="px-4 py-6 sm:px-6 lg:px-8">
      <form onSubmit={onSubmit} className="mx-auto flex max-w-3xl flex-col gap-4" autoComplete="off">
        ...
        {submitSuccess && (
          <div className="flex items-start gap-2.5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-[13px] text-emerald-800">
        ...
        <SectionCard step={1} title="Pick a device" description="Search by name, asset code, service tag or model.">
          {selectedDevice ? (
            <div className="flex items-center justify-between gap-3 rounded-lg border border-orange-200 bg-orange-50 px-3.5 py-2.5">
              <div className="min-w-0">
                <p className="truncate text-[13px] font-semibold text-slate-900">{selectedDevice.display_name}</p>
                <p className="mt-0.5 truncate text-xs text-slate-500">
